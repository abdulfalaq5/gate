...
// Function untuk validasi token dengan SSO server
async function validateTokenWithSSO(token) {
  try {
    const response = await fetch(`${SSO_CONFIG.serverUrl}/api/v1/auth/sso/userinfo`, {
      headers: {
    ...
    return userData.data; // Assuming SSO returns { success: true, data: {...} }
  } catch (error) {
    throw new Error('Failed to validate token with SSO server');
  }
}
...
// SSO Callback - DI SINI BEARER TOKEN DIDAPAT
app.get('/auth/callback', async (req, res) => {
  try {
    ...
    // STEP 1: Exchange code untuk access token (BEARER TOKEN)
    ...
    if (tokenData.access_token) {
      // STEP 2: Get user info menggunakan Bearer Token
      ...
      // STEP 3: Simpan Bearer Token di session
      req.session.user = userData;
      req.session.accessToken = tokenData.access_token; // INI BEARER TOKEN
    ...
    res.send(`
      ...
      <head>
          ...
          <style>
              ...
              .token-info { background: #e9ecef; padding: 15px; border-radius: 5px; margin: 20px 0; }
              .api-test { margin: 20px 0; }
              .api-test button { background: #28a745; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; }
              .api-test button:hover { background: #218838; }
          </style>
      </head>
      <body>
          ...
          <div class="token-info">
              <h3>Bearer Token Info</h3>
              <p><strong>Access Token:</strong> ${req.session.accessToken ? req.session.accessToken.substring(0, 20) + '...' : 'N/A'}</p>
              <p><strong>Token Type:</strong> Bearer</p>
              <p><strong>Expires In:</strong> 1 hour</p>
          </div>
          
          <div class="api-test">
              <h3>Test API dengan Bearer Token</h3>
...
// API endpoint yang membutuhkan Bearer Token
...
// API endpoint untuk mendapatkan user info (menggunakan session)
app.get('/api/user', (req, res) => {
  if (req.session.user) {
    res.json({
      success: true,
      user: req.session.user,
      access_token: req.session.accessToken ? req.session.accessToken.substring(0, 20) + '...' : null
    });
  } else {
    res.status(401).json({
      success: false,
...
// API endpoint untuk refresh token
app.post('/api/refresh-token', async (req, res) => {
  try {
    ...
    const refreshResponse = await fetch(`${SSO_CONFIG.serverUrl}/api/v1/auth/sso/token`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
